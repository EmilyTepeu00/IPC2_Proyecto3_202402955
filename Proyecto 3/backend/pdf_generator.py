...
def generar_analisis_ventas(fecha_inicio, fecha_fin, tipo_analisis, datos_ventas):
    try:
        ...
        #Crear nombre de archivo
        filename = f"analisis_ventas_{fecha_inicio}_{fecha_fin}_{tipo_analisis}.pdf"
        filepath = os.path.join(reportes_dir, filename)
        ...
        if tipo_analisis == "categorias":
            ...
            for categoria in datos_ventas.get('categorias', []):
                if categoria['ingresos'] > 0:  #Solo mostrar categorias con ingresos
                    cat_title = Paragraph(f"<b>Categoria: {categoria.get('nombre', '')}</b>", styles['Heading2'])
                    elements.append(cat_title)
                    
                    cat_data = [['Configuracion', 'Ingresos', '% del Total']]
                    total_categoria = categoria['ingresos']
                    
                    for config in categoria.get('configuraciones', []):
                        if config['ingresos'] > 0:  #Solo mostrar configuraciones con ingresos
                    ...
                    if cat_data.length > 1:  #Si hay datos ademas del encabezado
                        porcentaje_cat = (total_categoria / datos_ventas.get('total_facturado', 1)) * 100 if datos_ventas.get('total_facturado', 0) > 0 else 0
                        cat_data.append([
                            '<b>TOTAL CATEGORÍA</b>',
                            f"<b>{total_categoria:.2f}</b>",
                            f"<b>{porcentaje_cat:.1f}%</b>"
                        ])
            ...
            for recurso in datos_ventas.get('recursos', []):
                if recurso['ingresos'] > 0:  #Solo mostrar recursos con ingresos
            ...
            if len(recursos_data) > 1:  #Si hay datos ademas del encabezado
        ...
        #Generar PDF
        doc.build(elements)
        print(f"Analisis de ventas generado: {filepath}")
        return filepath
